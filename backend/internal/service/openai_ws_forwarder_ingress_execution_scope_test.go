...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_StateBoundToExecutionScope(t *testing.T) {
	...
	_, turnStateOnLegacy := stateStore.GetSessionTurnState(groupID, legacyHash, openAICodexTurnStateCredentialKey(nil, account))
	require.False(t, turnStateOnLegacy, "turn state 不得绑定到会话哈希")
	turnState, turnStateOnScope := stateStore.GetSessionTurnState(groupID, scope, openAICodexTurnStateCredentialKey(nil, account))
