...
func TestRealtimeModerationRetainsRemindersForKeywordChecks(t *testing.T) {
	for _, body := range []string{
		`{"type":"session.update","session":{"instructions":"<system-reminder>keyword</system-reminder>"}}`,
		`{"type":"conversation.item.create","item":{"role":"user","content":[{"type":"input_text","text":"<system-reminder>keyword</system-reminder>"}]}}`,
		`{"type":"response.create","response":{"input":"<system-reminder>keyword</system-reminder>"}}`,
	} {
		require.Empty(t, ExtractContentModerationText(ContentModerationProtocolOpenAIRealtime, []byte(body)))
		require.Contains(t, extractContentModerationKeywordText(ContentModerationProtocolOpenAIRealtime, []byte(body)), "keyword")
	}
}
