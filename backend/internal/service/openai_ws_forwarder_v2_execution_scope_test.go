...
func TestOpenAIGatewayService_Forward_WSv2_TurnStateBoundToExecutionScope(t *testing.T) {
	...
	_, turnStateOnLegacy := store.GetSessionTurnState(groupID, legacyHash, openAICodexTurnStateCredentialKey(c, account))
	require.False(t, turnStateOnLegacy, "turn state 不得绑定到按 session_id 算出的会话哈希")
	turnState, turnStateOnScope := store.GetSessionTurnState(groupID, scope, openAICodexTurnStateCredentialKey(c, account))
...
func TestOpenAIGatewayService_Forward_WSv2_ExecutionScopeUsesOriginalIdentity(t *testing.T) {
	...
	_, boundA := stateStore.GetSessionTurnState(groupID, scopeA, openAICodexTurnStateCredentialKey(cA, account))
	require.True(t, boundA, "会话 A 的 turn state 应落在按原始 session_id 算出的作用域")
	_, boundB := stateStore.GetSessionTurnState(groupID, scopeB, openAICodexTurnStateCredentialKey(cB, account))
	...
	_, boundToInjected := stateStore.GetSessionTurnState(groupID, injectedScope, openAICodexTurnStateCredentialKey(cA, account))
	...
	_, boundC := stateStore.GetSessionTurnState(groupID, scopeC, openAICodexTurnStateCredentialKey(cC, account))
