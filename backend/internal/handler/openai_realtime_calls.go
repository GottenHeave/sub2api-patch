...
func (h *OpenAIGatewayHandler) RealtimeREST(c *gin.Context) {
	...
	selectionModel := parsed.Model
	if mapped := strings.TrimSpace(channelMappedModels[parsed.ScheduleModelPath]); mapped != "" {
	...
	routingCtx := h.gatewayService.WithOpenAIRealtimeCallAccount(c.Request.Context(), apiKey.GroupID, parsed.CallID)
	...
	for {
		selection, scheduleDecision, err := h.gatewayService.SelectAccountWithSchedulerForRealtimeREST(
			c.Request.Context(),
			apiKey.GroupID,
			sessionHash,
			selectionModel,
		...
		if err != nil {
			...
			if len(failedAccountIDs) == 0 {
				...
				h.handleStreamingAwareError(c, http.StatusServiceUnavailable, "api_error", "No available OpenAI API key or OAuth accounts supporting this Realtime request", streamStarted)
				return
			}
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				h.gatewayService.ReportOpenAIAccountScheduleResult(account, scheduledModel, false, nil, err)
				...
				if parsed.CallID != "" || switchCount >= maxAccountSwitches {
		...
		if result != nil {
			if callID, err := service.OpenAIRealtimeCallIDFromLocation(result.ResponseHeaders.Get("Location")); err == nil {
				if err := h.gatewayService.BindStickySession(c.Request.Context(), apiKey.GroupID, service.OpenAIRealtimeCallSessionHash(callID), account.ID); err != nil {
					reqLog.Warn("openai.realtime_rest.bind_call_account_failed", zap.Error(err))
				}
			}
