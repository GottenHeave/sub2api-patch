...
func TestOpenAIGatewayHandlerRealtimeREST_AcceptActionDispatchesAccount(t *testing.T) {
...
func TestOpenAIGatewayHandlerRealtimeREST_AcceptActionAccountMappingForwardsMappedModel(t *testing.T) {
	upstream := &realtimeCallsHandlerUpstream{statuses: []int{http.StatusOK}}
...
func TestOpenAIGatewayHandlerRealtimeREST_CreateCallAccountSwitch(t *testing.T) {
	...
	c, rec := newRealtimeCallsHandlerContext(t, "/v1/realtime/calls", []byte(`{"type":"realtime","model":"gpt-realtime"}`))
...
func TestOpenAIGatewayHandlerRealtimeREST_CallControlDoesNotSwitchAccounts(t *testing.T) {
	upstream := &realtimeCallsHandlerUpstream{statuses: []int{http.StatusTooManyRequests, http.StatusOK}}
	handler := newRealtimeCallsHandlerForTest(t, upstream, []service.Account{
		newRealtimeCallsHandlerAccount(1), newRealtimeCallsHandlerAccount(2),
	})
	c, rec := newRealtimeCallsHandlerContext(t, "/v1/realtime/calls/call_123/reject", []byte(`{"status_code":603}`))
	handler.RealtimeREST(c)
	require.NotEqual(t, http.StatusOK, rec.Code)
	require.Equal(t, []int64{1}, upstream.accountIDs)
}

func TestOpenAIGatewayHandlerRealtimeREST_SelectsAccountSupportingRequestedModel(t *testing.T) {
	upstream := &realtimeCallsHandlerUpstream{}
	unsupported := newRealtimeCallsHandlerAccount(1)
	unsupported.Credentials["model_mapping"] = map[string]any{"other-model": "other-model"}
	supported := newRealtimeCallsHandlerAccount(2)
	supported.Credentials["model_mapping"] = map[string]any{"client-realtime": "gpt-realtime"}
	handler := newRealtimeCallsHandlerForTest(t, upstream, []service.Account{unsupported, supported})
	c, rec := newRealtimeCallsHandlerContext(t, "/v1/realtime/calls", []byte(`{"session":{"model":"client-realtime"}}`))
	handler.RealtimeREST(c)
	require.Equal(t, http.StatusOK, rec.Code)
	require.Equal(t, []int64{2}, upstream.accountIDs)
	require.Equal(t, "gpt-realtime", gjson.GetBytes(upstream.bodies[0], "session.model").String())
}

func TestOpenAIGatewayHandlerRealtimeREST_SkipsUnsupportedAccountTypes(t *testing.T) {
	upstream := &realtimeCallsHandlerUpstream{statuses: []int{http.StatusOK}}
	unsupported := newRealtimeCallsHandlerAccount(1)
	unsupported.Type = service.AccountTypeSetupToken
	unsupported.Credentials = map[string]any{"access_token": "setup-token"}
	handler := newRealtimeCallsHandlerForTest(t, upstream, []service.Account{
		unsupported,
		newRealtimeCallsHandlerAccount(2),
	})
...
func TestOpenAIGatewayHandlerRealtimeREST_OAuthCallUsesBackend(t *testing.T) {
	upstream := &realtimeCallsHandlerUpstream{}
	account := newRealtimeCallsHandlerAccount(1)
	...
	handler := newRealtimeCallsHandlerForTest(t, upstream, []service.Account{account})
	c, rec := newRealtimeCallsHandlerContext(t, "/v1/realtime/calls", []byte(`{"sdp":"v=0","session":{"model":"client-realtime"}}`))
	handler.RealtimeREST(c)
	require.Equal(t, http.StatusOK, rec.Code)
	require.Equal(t, []int64{1}, upstream.accountIDs)
	require.Equal(t, []string{"https://chatgpt.com/backend-api/codex/realtime/calls"}, upstream.urls)
	require.Equal(t, "gpt-realtime", gjson.GetBytes(upstream.bodies[0], "session.model").String())
}
...
func TestOpenAIGatewayHandlerRealtimeREST_TranslationClientSecretFiltersBySessionModel(t *testing.T) {
	upstream := &realtimeCallsHandlerUpstream{statuses: []int{http.StatusOK}}
	account := newRealtimeCallsHandlerAccount(1)
	account.Credentials = map[string]any{
		"api_key": "test-key",
