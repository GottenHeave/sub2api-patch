...
func (h *OpenAIGatewayHandler) AudioTranscriptions(c *gin.Context) {
	...
	routingCtx := service.WithModelRateLimitExtraScopes(
		...
		service.OpenAIAudioTranscriptionsModelRateLimitScopeIfModel(service.OpenAIAudioTranscriptionsAccountSelectionModel(parsed.Model)),
	)
	...
	for {
		...
		if parsed.IsVoiceInkProbe() {
			accountReleaseFunc()
			h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", "Invalid probe audio")
			return
		}
