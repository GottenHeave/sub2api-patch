...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&audioInputTokens,
			&audioOutputTokens,
			&audioCacheCreationTokens,
			&audioCacheReadTokens,
			&cost,
			&actualCost,
			&accountCost,
			&averageDurationMs,
		...
		totalTokens := inputTokens + outputTokens + cacheCreationTokens + cacheReads + audioInputTokens + audioOutputTokens + audioCacheCreationTokens + audioCacheReadTokens
